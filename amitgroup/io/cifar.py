...
def load_cifar10(section, classes=None, selection=None,  x_dtype = np.float64):
    assert section in ['training', 'testing']
    if section == 'training':
        data = np.load(os.path.join(os.environ['CIFAR10'], 'cifar10TrainingData.npy'))
        label = np.load(os.path.join(os.environ['CIFAR10'], 'cifar10TrainingDataLabel.npy'))
    elif section == 'testing':
        data = np.load(os.path.join(os.environ['CIFAR10'], 'cifar10TestingData.npy'))
        label = np.load(os.path.join(os.environ['CIFAR10'], 'cifar10TestingDataLabel.npy'))

    if x_dtype in [np.float16, np.float32, np.float64]:
        data = data.astype(x_dtype) / 255.0
    ...
    if classes!=None:
        indices = [k for k in range(size) if label[k] in classes]
    else:
        indices = range(size)
    ...
    return np.rollaxis(data[indices].reshape(len(indices),3,32,32),1,4), label[indices]
