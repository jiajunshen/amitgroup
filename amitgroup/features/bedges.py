...
def colorEdges(images, episilon = 0.01):
    ...
    sumImages = np.sum(images, axis = -1)[:,:,:,np.newaxis] + 3 * episilon
    processedImage = (images + episilon)/sumImages
    
    colorFeature = np.zeros(processedImage.shape+(3,))
    colorFeature[processedImage<1/3.0][0] = 1
    colorFeature[(processedImage<2/3.0)&(processedImage>=1/3.0)][1] = 1
    colorFeature[processedImage>=1/3.0][1] = 1
